...
import { getSightsPageDetail } from "@/lib/getSightsPage";
import { Metadata } from "next";
import Link from "next/link";
import React from "react";
import Image from "next/image";
import { getSightsPageNav } from "@/lib/getSightsPage";
...
export async function generateMetadata({
  ...
  const getData = await getSightsPageDetail(params.slug);
...
export default async function page({ params }: PageProps) {
  const nav = await getSightsPageNav();
  const data = await getSightsPageDetail(params.slug);
    ...
    <div>
      ...
      {data.page.standard_page.standardPageContent.map(
        (block: any, index: number) => {
          if (
            block.__typename === `Page_StandardPage_StandardPageContent_ImgCol`
          ) {
            return (
            ...
            block.__typename === `Page_StandardPage_StandardPageContent_ImgMob`
          ) {
            return (
